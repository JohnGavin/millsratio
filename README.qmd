...
# Load tested examples from targets
tar_load(example_comparison)
tar_load(example_paradox)
tar_load(example_properties)
```
...
rix(
  date = "2026-01-05",
  r_pkgs = c("dplyr", "ggplot2", "plotly", "scales", "shiny", "tidyr"),
  git_pkgs = list(
    list(
      package_name = "millsratio",
      repo_url = "https://github.com/JohnGavin/millsratio",
      commit = "04d90e3cfd04585164a244fd9e07754addef46d5"
    )
  ),
  ide = "none",
  project_path = "."
...
```{r visualization, fig.width=8, fig.height=5, eval=FALSE}
...
```{r paradox_plot, fig.width=8, fig.height=5, eval=FALSE}
...
# For development (if devtools available):
...
```{r project-structure, eval=FALSE, echo=FALSE, comment=""}
...
# Filter out common build artifacts and large directories
tree_output <- tree_output[!grepl(
  "nix-shell-root|docs/|man/|Meta/|help/|inst/doc|default\\.nix|renv/|_targets/",
  tree_output
)]
